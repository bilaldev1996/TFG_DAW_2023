...
<main class="login-container flex shadow-lg">
    <form action="index.php?controller=CentroController&action=logearCentro" method="post">
        <h2><i class="fas fa-school"></i> Acceso Centro</h2>
        <div class="w-full">
            <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="email">
                Email
            </label>
            <input class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 mb-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="email" type="email" name="email" placeholder="tucentro@gmail.com" required>
...
<div data-te-modal-init class="fixed top-0 left-0 z-[1055] hidden h-full w-full overflow-y-auto overflow-x-hidden outline-none" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div data-te-modal-dialog-ref class="pointer-events-none relative w-auto translate-y-[-50px] opacity-0 transition-all duration-300 ease-in-out min-[576px]:mx-auto min-[576px]:mt-7 min-[576px]:max-w-[500px]">
        <div class="min-[576px]:shadow-[0_0.5rem_1rem_rgba(#000, 0.15)] pointer-events-auto relative flex w-full flex-col rounded-md border-none bg-white bg-clip-padding text-current shadow-lg outline-none dark:bg-white-600">
            ...
            <form action="index.php?controller=CentroController&action=enviarEmail" class="relative flex-auto p-4" data-te-modal-body-ref method="post" id="formEnvioEmail">
                <div class="mb-4">
...
?>
    <script>
        Swal.fire({
            icon: 'error',
            title: 'Oops...',
            text: 'No existe ningún centro con ese email!',
            timer: 2000,
            showConfirmButton: false,
        });
    </script>
